            ...
            if (window.store.getState().BM.OfficeOneFolders !== undefined && window.store.getState().BM.instanceName === "not selected") {
                if (Object.keys(window.store.getState().BM.OfficeOneFolders).length > 0)
                    ...
                    driveData = <div className="LIST_ITEM">
                        <p> In Ihrem Google Drive wurde noch kein OfficeOne.Office Ordner gefunden, soll ein neuer Ordner angelegt werden?</p>
                        <button onClick={this.handleFolderCreate} >Neuen Ordner "2019 OfficeOne.Office" in Goolge Drive anlegen</button>
                    </div>
            }
        } catch (e) {
